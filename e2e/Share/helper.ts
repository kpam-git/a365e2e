...
export class Helper {
  ...
  public static domStablizationTimeout: number = 1000;
  public static elementWaitTimeout: number = 60000;
  ...
  public static async Hoveron(element: string, morebtn: string): Promise<void> {
  ...
  public static async readJson(pageObject: string, key: string): Promise<string> {
    var currentUrl = await browser.getCurrentUrl();
    if (!this.jsonlocator) {
      this.jsonlocator = JSON.parse(fs.readFileSync(__dirname + '/locator.json', 'utf8'));
    }
    let expression = this.jsonlocator['pageobject'][pageObject][key];
    return expression;
  }
